    ...
    while(1)
    {
        ...
        if(packet->stream_index==audioindex)
        {
            ...
            while( avcodec_receive_frame(acodecCtx, frame) == 0)
            {
                ...
                qDebug()<<"decode bytecount size:"<<byteCnt;
                ...
                currentMs = av_q2d(pFmtCtx->streams[audioindex]->time_base)*1000*frame->pts;    // 需要所属媒体流AVStream的time_base的基来换算出当前显示的标准时间
                qDebug()<<"time:"<<destMs<<currentMs;
