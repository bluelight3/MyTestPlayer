...
void FFmpegThread::run()
{
    ...
    while (!stopped) {
        ...
        if (isPlay) {
            this->init();
            destMs = av_q2d(formatCtx->streams[videoStreamIndex]->time_base)*1000*formatCtx->streams[videoStreamIndex]->duration;
            isPlay = false;
            continue;
        }
        ...
        if (frameFinish >= 0) {
            ...
            if (index == videoStreamIndex) {
                ...
                int currentMs = av_q2d(formatCtx->streams[videoStreamIndex]->time_base)*1000*yuvFrame->pts;
                qDebug()<<destMs<<currentMs;
                if (currentMs > 0)  //目前因为.h265文件解码出来不存在时间戳 这里暂时先假设非裸流得到的时间戳>0
...
FFmpegWidget::FFmpegWidget(QWidget *parent) : QWidget(parent)
{
    ...
    image = QImage();
    lbl_duration = new QLabel(parent);
    lbl_duration->setStyleSheet("QLabel{border:2px solid green}");
    lbl_duration->setText("00:00:00/00:00:00");
    QRect qRect = parent->rect();
    qDebug()<<qRect.width()<<" "<<qRect.height();
    lbl_duration->setGeometry(qRect.width()-140, qRect.height()-20, 140, 20);
    lbl_duration->show();
...
void FFmpegWidget::onDuration(int currentMs,int destMs)      //时长
{
    ...
    qDebug()<<"onDuration："<<currentMs<<destMs;
    ...
    this->lbl_duration->setText(currentTime+"/"+destTime);
